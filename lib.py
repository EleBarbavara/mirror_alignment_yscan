...
def choose_tz(n_subscan, timestep, num_feed):
    '''
    if int(n_subscan)<11:
        tz = [str(-4)]*timestep*num_feed
    if int(n_subscan)>10 and int(n_subscan)<21:
        tz = [str(-5)]*timestep*num_feed
    if int(n_subscan)>20 and int(n_subscan)<31:
        tz = [str(-6)]*timestep*num_feed
    if int(n_subscan)>30 and int(n_subscan)<41:
        tz = [str(-7)]*timestep*num_feed
    if int(n_subscan)>40 and int(n_subscan)<51:
        tz = [str(-8)]*timestep*num_feed
    if int(n_subscan)>50 and int(n_subscan)<61:
        tz = [str(-9)]*timestep*num_feed
    if int(n_subscan)>60 and int(n_subscan)<71:
        tz = [str(-10)]*timestep*num_feed
    if int(n_subscan)>70 and int(n_subscan)<81:
        tz = [str(-11)]*timestep*num_feed
    '''
    if int(n_subscan)<11:
        tz = [str(-3)]*timestep*num_feed
    if int(n_subscan)>10 and int(n_subscan)<21:
        tz = [str(-4)]*timestep*num_feed
    if int(n_subscan)>20 and int(n_subscan)<31:
        tz = [str(-5)]*timestep*num_feed
    ...
    if int(n_subscan)>70 and int(n_subscan)<81:
        tz = [str(-10)]*timestep*num_feed
    if int(n_subscan)>80 and int(n_subscan)<91:
        tz = [str(-11)]*timestep*num_feed
    
    return tz

def choose_ty(n_subscan, ss_in, timestep, num_feed, range_pos, num_pos):
    n_subscan = int(n_subscan)
    n_subscan -=10
    if int(n_subscan)<ss_in+10:
        ty = [str(range_pos[0])]*timestep*num_feed
    if int(n_subscan)>ss_in+9 and int(n_subscan)<ss_in+20:
        ty = [str(range_pos[1])]*timestep*num_feed
    if int(n_subscan)>ss_in+19 and int(n_subscan)<ss_in+30:
        ty = [str(range_pos[2])]*timestep*num_feed
    if num_pos >3:    
        if int(n_subscan)>ss_in+29 and int(n_subscan)<ss_in+40:
            ty = [str(range_pos[3])]*timestep*num_feed
        if num_pos >4:
            if int(n_subscan)>ss_in+39 and int(n_subscan)<ss_in+50:
                ty = [str(range_pos[4])]*timestep*num_feed
            if num_pos >5:
                if int(n_subscan)>ss_in+49 and int(n_subscan)<ss_in+60:
                    ty = [str(range_pos[5])]*timestep*num_feed
                if num_pos >6:
                    if int(n_subscan)>ss_in+59 and int(n_subscan)<ss_in+70:
                        ty = [str(range_pos[6])]*timestep*num_feed
                    if num_pos >7:
                        if int(n_subscan)>ss_in+69 and int(n_subscan)<ss_in+80:
                            ty = [str(range_pos[7])]*timestep*num_feed
                        if num_pos >8:
                            if int(n_subscan)>ss_in+79 and int(n_subscan)<ss_in+92:
                                ty = [str(range_pos[8])]*timestep*num_feed
                            if num_pos >9:
                                if int(n_subscan)>ss_in+89 and int(n_subscan)<ss_in+100:
                                    ty = [str(range_pos[9])]*timestep*num_feed
...
def lindetrend(x, time, mode='cutted'):
    ...
    if mode == 'cutted':
        ...
        tot_mask = np.zeros(len(x)) 
        tot_mask[idx1:idx2] = 1 
        tot_mask = tot_mask.astype(bool)
        time = np.array(time)
        xdata = np.ma.masked_array(time, mask=tot_mask) #time[tot_mask]
        ydata = np.ma.masked_array(x, mask=tot_mask) #x[tot_mask]
        ...
        plt.plot(x)
        plt.plot(ydata)
        plt.plot(tot_mask*0.02)
        plt.plot(lin_func(time, *parfit))
        plt.plot(res)
    ...
    if mode == 'sigma':
        pixel_mask = []
        filtered_data = sigma_clip(x, sigma=5, maxiters=10) #result: masked array = masked array + mask (true = wanted values, false = masked values)
...
def plot_map(mapp, figdim, cfg, namefile='', name='', wcs=None, beam=None, halo=None): 
    if wcs==None:
